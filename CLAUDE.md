...
This project uses Phoenix LiveView 1.x, Ash Framework with Spark DSL, and Typst for PDF generation. When exploring patterns, prefer reading existing code in the current codebase over git history or deleted files.
...
When adding new data keys or fields, always verify the key names match between the data source (Elixir assigns/maps) and the consuming template (LiveView HEEx, Typst). Check both producer and consumer before considering the task done.

## Typst / PDF Reports

When working with Typst templates that receive data from Elixir/JSON, always handle type coercion explicitly. Booleans and numbers may arrive as strings and need conversion before comparison or display.

## Database Operations

When implementing database cleanup or bulk operations, always preview the affected records with a dry-run query first and confirm the filter isn't too broad before executing destructive operations.

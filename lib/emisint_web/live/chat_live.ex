defmodule EmisintWeb.ChatLive do
  use Elixir.EmisintWeb, :live_view
  ...
  def mount(_params, _session, socket) do
    ...
    EmisintWeb.Endpoint.subscribe("chat:conversations:#{socket.assigns.current_user.id}")
    ...
    socket =
      ...
      |> stream(
        :conversations,
        Emisint.Chat.my_conversations!(actor: socket.assigns.current_user)
      )
      |> assign(:messages, [])
  ...
  def handle_params(%{"conversation_id" => conversation_id}, _, socket) do
    conversation =
      Emisint.Chat.get_conversation!(conversation_id, actor: socket.assigns.current_user)
    ...
    cond do
      ...
      socket.assigns[:conversation] ->
        EmisintWeb.Endpoint.unsubscribe("chat:messages:#{socket.assigns.conversation.id}")
        EmisintWeb.Endpoint.subscribe("chat:messages:#{conversation.id}")
    ...
    |> stream(:messages, Emisint.Chat.message_history!(conversation.id, stream?: true))
    |> assign_message_form()
    |> then(&{:noreply, &1})
  end
  ...
  defp assign_message_form(socket) do
    form =
      if socket.assigns.conversation do
        Emisint.Chat.form_to_create_message(
          actor: socket.assigns.current_user,
          private_arguments: %{conversation_id: socket.assigns.conversation.id}
        )
        |> to_form()
      else
        Emisint.Chat.form_to_create_message(actor: socket.assigns.current_user)
        |> to_form()
      end

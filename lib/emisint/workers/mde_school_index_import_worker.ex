defmodule Emisint.Workers.MdeSchoolIndexImportWorker do
  @moduledoc """
  Oban worker that runs `MdeSchoolIndexImporter.import_file/1` in the background.
  ...
  alias Emisint.Assessments.MdeSchoolIndexImporter

  @pubsub_topic "school_index_import"

  @impl Oban.Worker
  def perform(%Oban.Job{args: %{"bucket" => _bucket, "key" => key} = args}) do
    tmp_path =
      Path.join(System.tmp_dir!(), "school_index_#{System.unique_integer([:positive])}.csv")

    log_id = Map.get(args, "log_id")

    try do
      Emisint.Storage.download_to_file!(key, tmp_path)
      result = MdeSchoolIndexImporter.import_file(tmp_path)

      case result do
        {:ok, stats} ->
          Logger.info(
            "[MdeSchoolIndexImportWorker] Completed #{Path.basename(key)} — " <>
          ...
          Phoenix.PubSub.broadcast(
            Emisint.PubSub,
            @pubsub_topic,
            {:school_index_import_completed, stats}
          )
        ...
        {:error, reason} ->
          Logger.error(
            "[MdeSchoolIndexImportWorker] Failed #{Path.basename(key)}: #{reason}"
          )
          ...
          Phoenix.PubSub.broadcast(
            Emisint.PubSub,
            @pubsub_topic,
            {:school_index_import_failed, reason}
          )

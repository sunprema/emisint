defmodule Emisint.Chat.Message do
  use Ash.Resource,
    otp_app: :emisint,
    domain: Emisint.Chat,
    extensions: [AshOban],
  ...
  oban do
    triggers do
      trigger :respond do
        actor_persister Emisint.AiAgentActorPersister
        action :respond
        queue :chat_responses
        lock_for_update? false
        scheduler_cron false
        worker_module_name Emisint.Chat.Message.Workers.Respond
        scheduler_module_name Emisint.Chat.Message.Schedulers.Respond
  ...
  actions do
    ...
    create :create do
      ...
      change Emisint.Chat.Message.Changes.CreateConversationIfNotProvided
    ...
    update :respond do
      ...
      transaction? false
      change Emisint.Chat.Message.Changes.Respond
    end
  ...
  attributes do
    ...
    attribute :source, Emisint.Chat.Message.Types.Source do
      allow_nil? false
      public? true
      default :user
    end
  ...
  relationships do
    belongs_to :conversation, Emisint.Chat.Conversation do
      public? true
      allow_nil? false
    end
